...
from consignment_app.models import Login, AddConsignment, AddTrack,Product
...
def customer_name(request):
    products = Product.objects.all()  # Retrieve all products from Product model
    print(products)  # Debug print to check products in the view
    return render(request, 'addConsignment.html', {'products': products})
...
def addConsignment(request):
    if request.method == "POST":
        ...
        smtp_server.ehlo()
        smtp_server.login('jyotihosamani58@gmail.com', 'auaymflsntcgrcoe')

        # Send the email
        try:
            msg = MIMEMultipart()
            msg['From'] = 'Consignment <jyotihosamani58@gmail.com>'
            msg['To'] = rec_email
            msg['Subject'] = email_subject
            msg.attach(MIMEText(email_body, 'plain'))

            smtp_server.sendmail('jyotihosamani58@gmail.com', rec_email, msg.as_string())
...
def search_results(request):
    ...
    if tracker_id:
        try:
            ...
            if trackers.exists():
                return render(request, 'search_results.html', {'trackers': trackers})
            else:
        ...
        return render(request, 'search_results.html', {'message': "Please enter a tracker ID."})
